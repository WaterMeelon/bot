...
exports.run = async (client, message, args) => {
  ...
  if (times !== null && day - (Date.now() - times) > 0) {
    ...
    message.channel.send({ content: `Bu gün **${ödül} wm** kazandın! Bu komutu **${time.hours ? time.hours + "saat" : ""} ${time.minutes ? time.minutes + "dakika" : ""} ${time.seconds ? time.seconds + "saniye**": ""} sonra tekrar kullanabilirsin.`, reply: { messageReference: message.id }});
