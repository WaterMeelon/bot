...
exports.run = async (client, message, args) => {
  ...
  if (Covid === "kazandın") {
    ...
    message.channel.send({ content: `<a:wmtik:910819726157049877> Yaşasın Yazı Çıktı **kazandın**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
  }
  if (Covid === "kaybettin") {
    db.çıkar(`para_${message.author.id}`, parseInt(bahis))
    message.channel.send({ content: `<a:wmhayr:910860239228776518> Olamaz Tura Çıktı **kaybettin**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
  }
...
exports.conf = {
  ...
  aliases: ["yt-yazı", "yazı"], //kısayollar
  permLevel: 0 //perm level mainde karşıliklar yazar
};

exports.help = {
  name: "yazıturayazı", //komutu çalıştıracak olan kelime
