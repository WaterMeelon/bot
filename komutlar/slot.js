...
exports.run = async (client, message, args) => {
  ...
  var çalmayın = [":watermelon:", ":banana:", ":eggplant:"]
  ...
  if(slot1 === slot2 && slot1 === slot3) {
    if(slot1 === "watermelon") {
      db.topla(`para_${message.author.id}`, 5 * parseInt(bahis))
      db.yaz(`slotsüresi_${message.author.id}`, Date.now());
      return message.channel.send({ content: `
      **Slot 1:** ${slot1}, **Slot 2:** ${slot2}, **Slot3:** ${slot3}
      <a:cekilis:874546543372406814> Yaşasın oynadığın bahsin **5 katını kazandın**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
    }
    if(slot1 === "muz") {
    ...
    return message.channel.send({ content: `
    **Slot 1:** ${slot1}, **Slot 2:** ${slot2}, **Slot3:** ${slot3}
    <:thumbsdown:864893024416956456> Olamaz bahisi **kaybettin**! Oynanan bahis: ${bahis}`, reply: { messageReference: message.id } });
  }
}
exports.conf = {
